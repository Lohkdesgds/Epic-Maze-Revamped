			...
			while (data.now != LSW_S_EXIT)
			{
				...
				case LSW_S_INITIALIZING:
				{
					...
					while (!loaded_all)
					{
						...
						drawText(font, color(1, 1, 0), 1, 1, 0, "FPS: %.2f - %02.3f%c", nowFps, (99.9f*ref_loading_files / LSW_BMP_MAX), '%');
					...
					for (double to_add = 0.0; to_add < 90.0; to_add += multiplier()) {
						...
						drawText(font, color(1, 1, 0), 1, 1, 0, "FPS: %.2f - %02.3f%c", nowFps, 100.0, '%');
				...
				case LSW_S_MENU:
				{
					...
					for (double pos = -1.55; pos < -0.02; pos += multiply * (0.2*fabsf(pos / 2.0)))
					{
						...
						drawText(font, color(1, 1, 0), 1, 1, 0, "FPS: %.2f | TIMEMULTIPLY: %.3f | POSRN: %.4f", nowFps, nowMult, pos);
					...
					while (choosing)
					{
						...
						drawText(font, color(1, 1, 0), 1, 1, 0, "FPS: %.2f | TIMEMULTIPLY: %.3f", nowFps, nowMult);
						drawIt(vig, 0.0, 0.0, 0);
						flip();
						multiply = multiplier();
					}
				...
				case LSW_S_GAMING:
				{
					...
					while (rn.playin)
					{
						...
						/* * * * * * * DRAWNING * * * * * * */
						{
							...
							drawText(font, color(1, 1, 0), 1, 1, 0, "FPS: %.2f TPS: %.3f P: %.3fx%.3f / %.3fx%.3f (R); S: %.2fE-3, Sy: %.4fE-3; R: %.2f | CP [%.2f;%.2f] | K: %d | T: %04.1f", gr.nowFps, gr.collision_tps, gr.posrel[0], gr.posrel[1], gr.cammult[0], gr.cammult[1], gr.speed*1000.0, gr.speed_y*1000.0, gr.plrrot, gr.campos[0], gr.campos[1], data.evm.getLastKeyIfAvailable(LSW_KEYKEEP_LAST), gr.distance_taken);
						...
						if (mapread <= 0 && !gr.asked_for_help) {
							for (int t = 0; t < 9; t++) {
									...
									(fabs((gr.posrel[1] - gr.campos[1]) - (yy + gr.blocksize_rel[1])) <= gr.min_dist[t][1]))
								{
									...
									if (gr.debug > 0) al_draw_filled_circle(((gr.posrel[0] - gr.campos[0] - gr.min_dist[t][0])*0.5 + 0.5) * data.usedx, ((gr.posrel[1] - gr.campos[1] - gr.min_dist[t][1])*0.5 + 0.5) * data.usedy, 50.0, color(1, 0, 1));
									t = 9;
