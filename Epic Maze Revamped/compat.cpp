...
const bool load()
{
	return al_init() ? (al_install_keyboard() ? (al_init_primitives_addon() ? (al_init_font_addon() ? (al_init_ttf_addon() ? (al_init_image_addon() ? true : false) : false) : false) : false) : false) : false;
...
const int getBitmapSize(const lsw_texture u, const int axes)
{
	...
	else return al_get_bitmap_height(u);
}
...
const int getDisplaySize(const lsw_display u, const int axes)
{
	...
	else return al_get_display_height(u);
}
